...
public class Thermometer extends View {
    ...
    private static final int NB_GRADUATIONS = 9;
    public static final float MAX_TEMP = 60, MIN_TEMP = 0;
    private static final float RANGE_TEMP = 60;
    private static final int NB_GRADUATIONS_F = 9;
    private static final float MAX_TEMP_F = 60, MIN_TEMP_F = 0;
    ...
    public void init(Context context, AttributeSet attrs) {
        ...
        degreePaint.setStrokeWidth(middleCircleRadius / 8);
        degreePaint.setColor(Color.parseColor("#BBBBBB"));
        ...
        graduationPaint.setTextSize(54);//(Utils.convertDpToPixel(GRADUATION_TEXT_SIZE, getContext()));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float innerEffectEndY = circleCenterY - outerCircleRadius - 5;
        ...
        rangeRect.left = circleCenterX - innerRectRadius;
        rangeRect.top = innerStartY;
        rangeRect.right = circleCenterX + innerRectRadius;
        rangeRect.bottom = circleCenterY;

        canvas.drawRoundRect(rangeRect, innerRectRadius, innerRectRadius, innerPaint);
       // canvas.drawRect(circleCenterX - innerRectRadius, innerStartY, circleCenterX + innerRectRadius, circleCenterY, innerPaint);
        ...
        Log.e("ifS:", String.valueOf(innerEffectStartY));

        Log.e("isE:", String.valueOf(innerEffectEndY));
        ...
        while (tmp <= innerEffectEndY) {
            if (progressCount == 0) {
                canvas.drawLine(circleCenterX - outerRectRadius - DEGREE_WIDTH - DEGREE_WIDTH - DEGREE_WIDTH/2, tmp, circleCenterX - outerRectRadius + 5, tmp, degreePaint);

            } else if (progressCount%3 == 0) {
                canvas.drawLine(circleCenterX - outerRectRadius - DEGREE_WIDTH - DEGREE_WIDTH - DEGREE_WIDTH/2, tmp, circleCenterX - outerRectRadius + 5, tmp, degreePaint);

            } else {
                canvas.drawLine(circleCenterX - outerRectRadius - DEGREE_WIDTH - DEGREE_WIDTH/2, tmp, circleCenterX - outerRectRadius + 5, tmp, degreePaint);
            }
            /*
            String txt = ((int) startGraduation) + "°";
            Log.e("tempTxt:", String.valueOf(txt));
